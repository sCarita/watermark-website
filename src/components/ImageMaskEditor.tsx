...
const ImageMaskEditor = ({
  ...
  const handleDrawingUrl = () => {
    ...
    // Transform semi-transparent red pixels to white
    for (let i = 0; i < imageData.data.length; i += 4) {
      // With opacity 0.3, the red color (155) becomes around 46-47 (155 * 0.3)
      // and the other colors (12, 60) become very small
      if (
        imageData.data[i + 3] > 0 && // has some opacity
        imageData.data[i] > 30 && // red component is significant
        imageData.data[i + 1] < 20 && // green is low
        imageData.data[i + 2] < 30 // blue is low
      ) {
